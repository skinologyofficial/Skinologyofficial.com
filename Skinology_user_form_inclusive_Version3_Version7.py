...
if st.button("ยืนยันและรับโปรโตคอลเฉพาะบุคคล"):
    ...
    # ใส่ URL จริงของ backend เมื่อ deploy แล้ว
    backend_url = "https://yourbackend.com/analyze/"  # เปลี่ยนเป็น public URL หลัง deploy backend จริง
    try:
        ...
        if res.status_code == 200:
            ...
            # PDF Download (แก้ path ให้ถูก)
            pdf_url = result_dict.get("pdf_url", "")
            if pdf_url:
                st.markdown(f"[ดาวน์โหลด PDF รายงานเฉพาะตัว]({pdf_url})")
        else:
