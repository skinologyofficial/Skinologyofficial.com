...
if st.button("ยืนยันและรับโปรโตคอลเฉพาะบุคคล"):
    files = {
        ...
        "body_image": body_image if body_image else None
    }
    # Filter out None
    files = {k: v for k, v in files.items() if v is not None}
    file_data = {k: (v.name, v.read(), v.type) for k, v in files.items()}
    ...
    # Change backend url as needed!
    backend_url = "http://localhost:8000/analyze/"
    res = requests.post(backend_url, files=file_data, data=data)
    if res.status_code == 200:
        result_dict = res.json()
        st.success("สร้างสำเร็จ! ผลลัพธ์ของคุณพร้อมแล้ว 🎉")
        st.write(result_dict["result"])
        st.markdown(f"ดาวน์โหลด PDF รายงานเฉพาะตัว [คลิกที่นี่]({backend_url.replace('/analyze/', f'/download/{result_dict['pdf_url'].split('/')[-1]}')})")
    else:
        st.error("ไม่สามารถสร้างผลลัพธ์ โปรดตรวจสอบรูปและข้อมูลที่ส่ง")